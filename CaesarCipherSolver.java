...
public class CaesarCipherSolver {
    ...
    public static void rot(List<String> sentence, int amount) {
        ...
        for (String word : sentence) {
            String rotatedWord = CaesarCipher.rot(word, amount);
            sentence.set(idx, rotatedWord);
            idx += 1;
        }
        }
    ...
    public static String putTogetherWithSpaces(List<String> words) {
        ...
        for (int i = words.size() - 1; i < words.size(); i++) {
            wordsString += words.get(i);
        }
    ...
    public static void main(String[] args) {
        ...
        List<String> lineList = splitBySpaces(line);
        for (int i = 0; i < 26; i++) {
            CaesarCipherSolver.rot(lineList, 1);
            if (howManyWordsIn(lineList,dictionary) > lineList.size() / 2) {
                System.out.println(putTogetherWithSpaces(lineList));
            }
        }
    }
